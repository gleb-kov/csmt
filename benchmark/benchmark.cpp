...
constexpr size_t DEF_KEYS = 100'000;
...
void spam_insert() {
    ...
    for (size_t idx = 0; idx < KEYS; ++idx) {
        ...
        uint64_t iter_elapsed_ns = st.stop_stage<std::chrono::microseconds>().count();
        elapsed_ns += iter_elapsed_ns;
        min_elapsed_ns = std::min(min_elapsed_ns, iter_elapsed_ns);
        max_elapsed_ns = std::max(max_elapsed_ns, iter_elapsed_ns);
    }
    std::cout << "Average elapsed: " << elapsed_ns * 1.0 / KEYS << " us." << std::endl;
    std::cout << "Min elapsed: " << min_elapsed_ns << " us." << std::endl;
    std::cout << "Max elapsed: " << max_elapsed_ns << " us." << std::endl;
}
...
void spam_all() {
    ...
    for (size_t idx = 0; idx < KEYS; ++idx) {
        ...
        uint64_t iter_elapsed_ns = 0;

        if (op == 0) {
            std::string val_str = string_utils::generate_random_string(VALUE_SIZE);
            st.start_stage();
            tree.insert(key_index, val_str);
            iter_elapsed_ns = st.stop_stage<std::chrono::microseconds>().count();
        } else if (op == 1) {
            st.start_stage();
            tree.erase(key_index);
            iter_elapsed_ns = st.stop_stage<std::chrono::microseconds>().count();
